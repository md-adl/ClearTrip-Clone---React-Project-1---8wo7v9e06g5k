  ...
  return (
    <Stack >
      ...
      >
        ...
        <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} px={{ xs: 2, md: 20 }} py={5}>
          <Stack spacing={5} sx={{ width: { xs: '100%', md: '30%' } }}>
          ...
          <Stack spacing={3} sx={{ width: { xs: '100%', md: '70%' } }}>
            <Stack direction="row" sx={{ backgroundColor: "#F7F7F7" }} p={2} boxShadow={1}>
              <Box width={{ xs: '40%', md: '25%' }} >Airlines</Box>
              <Box width={{ xs: '20%', md: '15%' }} sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>Departure</Box>
              <Box width={{ xs: '20%', md: '15%' }} sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>Duration</Box>
              <Box width={{ xs: '20%', md: '15%' }} sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>Arrival</Box>
              <Box width={{ xs: '20%', md: '15%' }} sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>Price</Box>
              <Box width={{ xs: '0%', md: '15%' }}></Box>
            </Stack>
            {
                ...
                <Collapse in={true} key={item.flightID}>
                  <Accordion >
                    <AccordionSummary
                      ...
                      sx={{ py: "2%" }}
                    >
                      <Stack width={{ xs: '100%', md: '25%' }} a >
                        <Box>Flight Id</Box>
                        <Box>{item.flightID}</Box>
                      </Stack>
                      <Box width={{ xs: '100%', md: '15%' }} sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>{item.departureTime}</Box>
                      <Stack width={{ xs: '100%', md: '15%' }} color="#999" divider={<Divider orientation="horizontal" width="50%" />} sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
                        <Box>{`${item.duration}h 0m`}</Box>
                        <Box>{item.stops == 0 ? "non stop" : item.stops < 2 ? `${item.stops} stop` : `${item.stops} stops`}</Box>
                      </Stack>
                      <Box width={{ xs: '100%', md: '15%' }} sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>{item.arrivalTime}</Box>
                      <Box width={{ xs: '100%', md: '15%' }} sx={{ display: "flex", justifyContent: "center", alignItems: "center", fontWeight: "bold" }}>₹{item.ticketPrice}</Box>
                    ...
                    <AccordionDetails>
                      <Stack divider={<Divider orientation="horizontal" width="100%" />} sx={{ border: "0.5px solid #e6e6e6", borderRadius: "5px", display: "flex", justifyContent: 'space-between' }}>
                        ...
                        <Stack direction="row" justifyContent="space-between" p={2}>
                          ...
                          <Stack direction="column" sx={{ fontSize: "12px" }} width={{ xs: '100%', md: '25%' }}>
